...
import net.dnadas.companies.dto.requests.CompanyJoinRequestUpdateDto;
import net.dnadas.companies.exception.company.CompanyJoinRequestNotFoundException;
import net.dnadas.companies.exception.company.CompanyNotFoundException;
import net.dnadas.companies.exception.company.DuplicateCompanyJoinRequestException;
import net.dnadas.companies.exception.company.UserAlreadyInCompanyException;
...
import net.dnadas.companies.model.request.CompanyJoinRequestDao;
...
public class CompanyRequestService {
  ...
  public CompanyJoinRequestResponseDto createJoinRequest(Long companyId) {
    ...
    if (company.getEmployees().contains(userInfo.userId())) {
      throw new UserAlreadyInCompanyException();
    }
    Optional<CompanyJoinRequest> duplicateRequest = requestDao.findOneByCompanyAndUserId(
      company, userInfo.userId());
    if (duplicateRequest.isPresent()) {
      throw new DuplicateCompanyJoinRequestException();
    }
    CompanyJoinRequest joinRequest = new CompanyJoinRequest(company, userInfo.userId());
    CompanyJoinRequest savedRequest = requestDao.save(joinRequest);
    return new CompanyJoinRequestResponseDto(
      savedRequest.getId(), companyConverter.getCompanyResponsePublicDto(company),
      new UserResponsePublicDto(userInfo.userId(), userInfo.username()), savedRequest.getStatus());
  }
